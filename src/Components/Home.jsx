...
function Home({Displayimages,navimg}) {
    ...
    const handleButtonClick = (event) => {
        ...
        setDisplayOutput(`You And Your Crush Are ${finalOutput} `)

    };

    return (
        <div className="flex font-bold  bg-gradient-to-b from-gray-900 via-slate-500 to-zinc-100 flex-col h-screen w-screen bg-contain bg-center bg-no-repeat md:flex-row "  >
                ...
                <div className='w-3/4'>
                    <form className='flex flex-col gap-4 w-full '>
                        <div className='flex flex-col gap-1 w-full '>
                            ...
                            <input className='h-12 bg-slate-50 rounded-lg w-full p-4' id="name1" type="text" placeholder='Your Name' value={name1} onChange={handleName1Change}></input>
                        </div>
                        <div className='flex flex-col gap-1'>
                            <label className='h-10 w-full' htmlFor="name2">Enter Your Crush Name</label>
                            <input className='h-12 bg-slate-50 rounded-lg w-full p-4 ' id="name2" type="text" placeholder='Your Crush Name' value={name2} onChange={handleName2Change}></input>
                        </div>
                        <div className='flex flex-col gap-1 bg-pink-500 rounded-md  justify-center items-center'>
                            <button className='h-10 w-2/4' type="submit" onClick={handleButtonClick}>Submit</button>
                        </div>
                    </form>
                </div>
            </div>
            <div className=' gap-5 h-2/4 w-screen text-5xl justify-center items-center flex md:w-3/4 md:h-screen flex-col'>
